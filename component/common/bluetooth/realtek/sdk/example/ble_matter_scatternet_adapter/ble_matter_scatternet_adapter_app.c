...
*****************************************************************************************
   * @file      ble_matter_scatternet_adapter_app.c
...
#include "ble_matter_scatternet_adapter_app_main.h"
//#include "ble_matter_scatternet_adapter_service.h"
...
int ble_matter_scatternet_adapter_peripheral_app_max_links = 0;
int ble_matter_scatternet_adapter_central_app_max_links = 0;

T_GAP_DEV_STATE ble_matter_scatternet_adapter_gap_dev_state = {0, 0, 0, 0, 0};                /**< GAP device state */
T_APP_LINK ble_matter_scatternet_adapter_app_link_table[BLE_MATTER_SCATTERNET_ADAPTER_APP_MAX_LINKS];

T_CLIENT_ID ble_matter_scatternet_adapter_gcs_client_id;         /**< General Common Services client client id*/
T_SERVER_ID ble_matter_scatternet_adapter_service_id;	/**< Matter service id */
...
void ble_matter_adapter_multi_adv_task_func(void *arg)
{
	...
	while (true) {
		if (os_msg_recv(matter_multi_adapter.queue_handle, &adv_id, 0xFFFFFFFF) == true) {
			...
			if (ble_matter_scatternet_adapter_app_send_api_msg(0, &matter_multi_adv_param_array[adv_id]) == false) {
 ...
 *                              Functions
 *============================================================================*/
int ble_matter_scatternet_adapter_app_handle_upstream_msg(uint16_t subtype, void *pdata)
{
    int ret = 0;
...
void ble_matter_scatternet_adapter_app_handle_callback_msg(T_IO_MSG callback_msg)
{
...
void ble_matter_scatternet_adapter_app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_BT_STATUS:
        {
            ble_matter_scatternet_adapter_app_handle_gap_msg(&io_msg);
        }
        break;
 ...
 * @return   void
 */
void ble_matter_scatternet_adapter_app_handle_dev_state_evt(T_GAP_DEV_STATE new_state, uint16_t cause)
{
	...
	if (ble_matter_scatternet_adapter_gap_dev_state.gap_init_state != new_state.gap_init_state) {
	...
	if (ble_matter_scatternet_adapter_gap_dev_state.gap_scan_state != new_state.gap_scan_state) {
	...
	if (ble_matter_scatternet_adapter_gap_dev_state.gap_adv_state != new_state.gap_adv_state) {
	...
	ble_matter_scatternet_adapter_gap_dev_state = new_state;
}
 ...
 * @return   void
 */
void ble_matter_scatternet_adapter_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
	T_GAP_CONN_INFO conn_info;
	if (conn_id >= BLE_MATTER_SCATTERNET_ADAPTER_APP_MAX_LINKS) {
		return;
	}

	APP_PRINT_INFO4("ble_matter_scatternet_adapter_app_handle_conn_state_evt: conn_id %d, conn_state(%d -> %d), disc_cause 0x%x",
					conn_id, ble_matter_scatternet_adapter_app_link_table[conn_id].conn_state, new_state, disc_cause);
	ble_matter_scatternet_adapter_app_link_table[conn_id].conn_state = new_state;
	switch (new_state) {
	case GAP_CONN_STATE_DISCONNECTED: {
		if ((disc_cause != (HCI_ERR | HCI_ERR_REMOTE_USER_TERMINATE))
			&& (disc_cause != (HCI_ERR | HCI_ERR_LOCAL_HOST_TERMINATE))) {
			APP_PRINT_ERROR2("ble_matter_scatternet_adapter_app_handle_conn_state_evt: connection lost, conn_id %d, cause 0x%x", conn_id,
							 disc_cause);
		}
		printf("Disconnect conn_id %d, cause 0x%x\r\n", conn_id, disc_cause);

		if (ble_matter_scatternet_adapter_app_link_table[conn_id].role == GAP_LINK_ROLE_MASTER) {
			ble_matter_scatternet_adapter_central_app_max_links --;
		} else if (ble_matter_scatternet_adapter_app_link_table[conn_id].role == GAP_LINK_ROLE_SLAVE) {
			ble_matter_scatternet_adapter_peripheral_app_max_links --;
		}

		if (ble_matter_scatternet_adapter_app_link_table[conn_id].role == GAP_LINK_ROLE_SLAVE) {
			printf("As peripheral,recieve disconncect,please start ADV\r\n");
		}

		memset(&ble_matter_scatternet_adapter_app_link_table[conn_id], 0, sizeof(T_APP_LINK));
#if CONFIG_BLE_MATTER_MULTI_ADV
		if (link_customer == 1) { //matter
			...
			if (ble_matter_scatternet_adapter_send_callback_msg(BLE_MATTER_MSG_DISCONNECTED_MULTI_ADV,NULL, disconnected_msg_matter) == false) {
	    ...
	    if(disconnected)
	    {
	        ...
	        if(ble_matter_scatternet_adapter_send_callback_msg(BT_MATTER_SEND_CB_MSG_DISCONNECTED, NULL, disconnected)==false)
	        {
	...
	case GAP_CONN_STATE_CONNECTED: {
		...
		ble_matter_scatternet_adapter_app_link_table[conn_id].role = conn_info.role;
		if (ble_matter_scatternet_adapter_app_link_table[conn_id].role == GAP_LINK_ROLE_MASTER) {
			ble_matter_scatternet_adapter_central_app_max_links ++;
		} else if (ble_matter_scatternet_adapter_app_link_table[conn_id].role == GAP_LINK_ROLE_SLAVE) {
			ble_matter_scatternet_adapter_peripheral_app_max_links ++;			}
        ...
        APP_PRINT_INFO5("GAP_CONN_STATE_CONNECTED:remote_bd %s, remote_addr_type %d, conn_interval 0x%x, conn_latency 0x%x, conn_supervision_timeout 0x%x",
                            TRACE_BDADDR(ble_matter_scatternet_adapter_app_link_table[conn_id].bd_addr), ble_matter_scatternet_adapter_app_link_table[conn_id].bd_type,
		...
		if (local_bd_type == GAP_LOCAL_ADDR_LE_RANDOM) { // matter
			...
			if (ble_matter_scatternet_adapter_send_callback_msg(BLE_MATTER_MSG_CONNECTED_MULTI_ADV, NULL, conn_msg_matter) == false) {
				printf("[%s] send callback msg fail\r\n", __func__);
 ...
 * @return   void
 */
void ble_matter_scatternet_adapter_app_handle_authen_state_evt(uint8_t conn_id, uint8_t new_state, uint16_t cause)
{
 ...
 * @return   void
 */
void ble_matter_scatternet_adapter_app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
 ...
 * @return   void
 */
void ble_matter_scatternet_adapter_app_handle_conn_param_update_evt(uint8_t conn_id, uint8_t status, uint16_t cause)
{
	switch (status) {
 ...
 * @return   void
 */
void ble_matter_scatternet_adapter_app_handle_gap_msg(T_IO_MSG *p_gap_msg)
{
	...
	case GAP_MSG_LE_DEV_STATE_CHANGE: {
		ble_matter_scatternet_adapter_app_handle_dev_state_evt(gap_msg.msg_data.gap_dev_state_change.new_state,
												gap_msg.msg_data.gap_dev_state_change.cause);
	}
	break;

	case GAP_MSG_LE_CONN_STATE_CHANGE: {
		ble_matter_scatternet_adapter_app_handle_conn_state_evt(gap_msg.msg_data.gap_conn_state_change.conn_id,
	...
	case GAP_MSG_LE_CONN_MTU_INFO: {
		ble_matter_scatternet_adapter_app_handle_conn_mtu_info_evt(gap_msg.msg_data.gap_conn_mtu_info.conn_id,
				gap_msg.msg_data.gap_conn_mtu_info.mtu_size);
	}
	break;

	case GAP_MSG_LE_CONN_PARAM_UPDATE: {
		ble_matter_scatternet_adapter_app_handle_conn_param_update_evt(gap_msg.msg_data.gap_conn_param_update.conn_id,
	...
	case GAP_MSG_LE_AUTHEN_STATE_CHANGE: {
		ble_matter_scatternet_adapter_app_handle_authen_state_evt(gap_msg.msg_data.gap_authen_state.conn_id,
...
/**
  ...
  * @retval void
  */
void bt_matter_scatternet_device_matter_app_parse_scan_info(T_LE_SCAN_INFO *scan_info)
{
    uint8_t buffer[32];
    uint8_t pos = 0;
...
void ble_matter_scatternet_adapter_gcs_handle_discovery_result(uint8_t conn_id, T_GCS_DISCOVERY_RESULT discov_result)
{
	uint16_t i;
...
T_APP_RESULT ble_matter_scatternet_adapter_gcs_client_callback(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data)
{
	T_APP_RESULT  result = APP_RESULT_SUCCESS;
	APP_PRINT_INFO2("ble_matter_scatternet_adapter_gcs_client_callback: client_id %d, conn_id %d",
					client_id, conn_id);
	if (client_id == ble_matter_scatternet_adapter_gcs_client_id) {
		...
		case GCS_CLIENT_CB_TYPE_DISC_RESULT:
			ble_matter_scatternet_adapter_gcs_handle_discovery_result(conn_id, p_gcs_cb_data->cb_content.discov_result);
			break;
...
T_APP_RESULT ble_matter_scatternet_adapter_app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
	T_APP_RESULT result = APP_RESULT_SUCCESS;
	...
	/* central reference msg*/
		...
		APP_PRINT_INFO5("GAP_MSG_LE_SCAN_INFO:adv_type 0x%x, bd_addr %s, remote_addr_type %d, rssi %d, data_len %d",
		    ...
		    bt_matter_scatternet_device_matter_app_parse_scan_info(p_data->p_le_scan_info);
	...
	default:
		APP_PRINT_ERROR1("ble_matter_scatternet_adapter_app_gap_callback: unhandled cb_type 0x%x", cb_type);
		break;
	}
	return result;
}

T_APP_RESULT ble_matter_scatternet_adapter_app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
...
//send msg to matter
				...
				if (p_param->event_data.send_data_result.service_id == ble_matter_scatternet_adapter_service_id)
				{
				    ...
				    if(send_data_complete)
				    {
					    ...
					    if(ble_matter_scatternet_adapter_send_callback_msg(BT_MATTER_SEND_CB_MSG_SEND_DATA_COMPLETE, service_id, send_data_complete)==false)
					    {
        ...
        case SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION:
           {
                ...
                {
                case MATTER_SCATTERNET_NOTIFY_INDICATE_V3_ENABLE:
                    {
                        ...
                        if(indication_notification_enable)
                        {
                            ...
                            if(ble_matter_scatternet_adapter_send_callback_msg(BLE_MATTER_MSG_CCCD_RECV_ENABLE_MULTI_ADV, service_id, indication_notification_enable)==false)
                            {
                ...
                case MATTER_SCATTERNET_NOTIFY_INDICATE_V3_DISABLE:
                    {
			    ...
			    if(ble_matter_scatternet_adapter_send_callback_msg(BLE_MATTER_MSG_CCCD_RECV_DISABLE_MULTI_ADV, service_id, indication_notification_disable)==false)
                            {
        ...
        case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE:
            {
                ...
                if(write_char_val)
                {
                    ...
                    if(ble_matter_scatternet_adapter_send_callback_msg(BLE_MATTER_MSG_WRITE_CHAR_MULTI_ADV, service_id, write_char_val)==false)
                    {
                        if (write_char_val->msg_data.write_read.len !=0)
                        {
                            os_mem_free(write_char_val->msg_data.write_read.p_value);
...
#if CONFIG_BLE_MATTER_MULTI_ADV
void ble_matter_scatternet_adapter_app_vendor_callback(uint8_t cb_type, void *p_cb_data)
{
...
T_APP_RESULT ble_matter_scatternet_adapter_gap_service_callback(T_SERVER_ID service_id, void *p_para)
{
	(void) service_id;
	T_APP_RESULT  result = APP_RESULT_SUCCESS;
